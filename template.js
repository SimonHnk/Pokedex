...
function insertExternalSearchPokemonCardHTML(j, id, pokemon, typeContainer) {
    return `
    <div onclick="openExternalSearchPokeCard(${j})" id="pokemonCard${id}" onmouseover="tiltShake(${id})" onmouseout="tiltShakeOff(${id})" class="card pokemon-card ${pokemon['types'][0]['type']['name']}">
        <div class="card-body">
        <div class="card-headline-container">
            <h4 id="pokemonName" class="card-title title-color title-font mb-3">${pokemon['name'].charAt(0).toUpperCase() + pokemon['name'].slice(1)}</h4>
            <h5>#${insertId(id)}</h5>
